...
const float th = 70e-3;
const float offset = 0.34*2.;
...
float symbol( in vec2 cs, in int id )
{
    float s = 0.;
    if (id == 0) {
      s = line(cs, vec2(0., 0.25-th), vec2(0.0, -0.25+th))-.25;
      s = abs(s)-th;
      s = max(s, sdBox(cs, vec2(0.332, 0.5)));
    } else if (id == 1) {
      s = line(cs, vec2(0., 0.5), vec2(0., -0.5))-th;
      s = max(s, sdBox(cs, vec2(0.332-th, 0.5)));
    } else if (id == 2) {
      s = line(cs, vec2(-0.02, 0.23-th), vec2(-0.6, -0.5));
      s = abs(s-0.27)-th;
      s = min(s, line(cs, vec2(0.312, -0.5+th), vec2(-0.2, -0.5+th))-th);
      s = max(s, sdBox(cs, vec2(0.312, 0.5)));
    } else if (id == 3) {
      s = line(cs, vec2(0.-th-0.04, 0.225), vec2(-0.332, 0.21));
      s = min(s, line(cs, vec2(0.-th-0.01, -0.225), vec2(-0.332, -0.21)));
      s = abs(s-0.225)-th;
      s = max(s, -sdBox(cs+vec2(0.2, 0.), vec2(th*1.1)));
      s = max(s, sdBox(cs, vec2(0.22, 0.5)));
    } else if (id == 4) {
      s = line(cs, vec2(0.332, -0.175), vec2(-0.332, -0.175));
      s = min(s, line(cs, vec2(-0.332, -0.175), vec2(0.227, 0.5)));
      s = min(s, line(cs, vec2(0.227, 0.5), vec2(0.227, -0.5)));
      s = abs(s)-th;
      s = max(s, sdBox(cs, vec2(0.332, 0.5)));
    } else if (id == 5) {
      s = line(cs, vec2(0.195+th, 0.5-th), vec2(-0.15, 0.5-th));
      s = min(s, line(cs, vec2(-0.23+th, 0.5), vec2(-0.245+th, 0.1)));
      s = min(s, abs(line(cs, vec2(-0.3, -0.25+th), vec2(0.0-th*1.1, -0.25+th))-0.25));
      s = abs(s)-th;
      s = max(s, sdBox(cs, vec2(0.245, 0.5)));
    } else if (id == 6) {
      s = length(cs+vec2(0., 0.275-th))-0.23;
      s = min(s, line(cs, vec2(0.09, 0.506), vec2(-0.197, -0.075)));
      s = abs(s)-th;
      s = max(s, sdBox(cs, vec2(0.332, 0.5)));
    } else if (id == 7) {
      s = line(cs, vec2(-0.319-th, 0.5-th), vec2(0.319+th, 0.5-th));
      s = min(s, line(cs, vec2(0.3, 0.5-th), vec2(-0.128, -0.5)));
      s = abs(s)-th;
      s = max(s, sdBox(cs, vec2(0.332, 0.5)));
    } else if (id == 8) {
      s = length(cs+vec2(0., 0.2))-0.25;
      s = min(s, length(cs+vec2(0., -0.25))-0.2);
      s = abs(s)-th;
      s = max(s, sdBox(cs, vec2(0.332, 0.5)));
    } else if (id == 9) {
      cs *= -1.;
      s = length(cs+vec2(0., 0.22))-0.23;
      s = min(s, line(cs, vec2(0.087, 0.5), vec2(-0.197, -0.1)));
      s = abs(s)-th;
      s = max(s, sdBox(cs, vec2(0.332, 0.5)));
    }
    return s;
...
void main()
{
    ...
    for (int i=0; i<1; i++)
    {
        tmp = symbol(tuv-vec2(offset*4.*float(i), 0.), i);
        tmp = 1. - smoothstep(-fwidth(tmp)*.75, fwidth(tmp)*.75, tmp);
        tmp = clamp(tmp, 0., 1.);
        number += tmp;
    }
    ...
    col = mix(col, SidesColors[0], number);
